...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> _loadChatHistories() async {
    try {
      _debugLog('Loading chat histories...');
      emit(state.copyWith(isLoading: true));

      final histories = await _chatRepository.getChatHistories();
      _debugLog('Loaded ${histories.length} chat histories');
      ...
      if (histories.isEmpty) {
        _debugLog('No chat histories found, resetting state completely');
        emit(ChatState(
          isLoading: false,
          error: null,
      ...
      _debugLog('Error loading chat histories: $e');
      emit(state.copyWith(
  ...
  void startNewChat() {
    ...
    _debugLog('New chat started with ID: $newChatId');
  }
  ...
  void _initializeChatStream() {
    ...
    if (state.currentChatId != null) {
      ...
      _debugLog('Initializing chat stream for chat: $chatId');
      ...
      _chatSubscription = _chatRepository.getChatMessagesForId(chatId).listen(
        (messages) {
          _debugLog('Received ${messages.length} messages for chat: $chatId');
        ...
        onError: (error) {
          _debugLog('Error in chat stream: $error');
      ...
      _debugLog('No current chat ID, clearing messages');
  ...
  Future<void> sendMessage({
    ...
    try {
      _debugLog('Sending message for chat: ${state.currentChatId}');
      ...
      if (state.currentChatId == null || state.currentChatId != chatId) {
        _debugLog('Updating current chat ID to: $chatId');
        emit(state.copyWith(currentChatId: chatId));
      }
      ...
      _debugLog('Message sent successfully to Firestore, chatId: $chatId');
      ...
      _debugLog('Error sending message: $e');
  ...
  void _updateMessagesState(String? chatId) {
    ...
    _debugLog('Updated messages state - count: ${sortedMessages.length}');
  }

  Future<void> deleteMessage(String messageId) async {
    try {
      _debugLog('Deleting message: $messageId');
      await _chatRepository.deleteMessage(messageId);
      await _loadChatHistories(); // Refresh chat histories after deletion
    } catch (e) {
      _debugLog('Error deleting message: $e');
  ...
  Future<void> forceDeleteChat(String chatId) async {
    try {
      _debugLog('Force deleting chat: $chatId');
      ...
      _debugLog('Successfully force deleted chat: $chatId');
    } catch (e) {
      _debugLog('Error force deleting chat: $e');
  ...
  Future<void> clearChat() async {
    try {
      _debugLog('Starting chat clear process in cubit');
      ...
      _debugLog('Repository clear operation completed');
      ...
      _debugLog('Chat clear process completed in cubit');
    } catch (e) {
      _debugLog('Error clearing chat in cubit: $e');
      emit(state.copyWith(
        isLoading: false,
  ...
  Future<void> pinChat(String chatId) async {
    try {
      ...
      _debugLog('Error pinning chat: $e');
  ...
  Future<void> unpinChat(String chatId) async {
    try {
      _debugLog('Unpinning chat: $chatId');
      if (state.pinnedChatIds.contains(chatId)) {
        final updatedPinnedChats = List<String>.from(state.pinnedChatIds)
          ..remove(chatId);
        emit(state.copyWith(pinnedChatIds: updatedPinnedChats));
      ...
      _debugLog('Error unpinning chat: $e');
  ...
  Future<void> deleteChat(String chatId) async {
    try {
      _debugLog('Deleting chat: $chatId');
      await _chatRepository.deleteChat(chatId);
      ...
      _debugLog('Error deleting chat: $e');
  ...
  Future<void> renameChat(String chatId, String newTitle) async {
    try {
      _debugLog('Renaming chat: $chatId to: $newTitle');
      await _chatRepository.updateChatTitle(chatId, newTitle);
      await _loadChatHistories();
    } catch (e) {
      _debugLog('Error renaming chat: $e');
  ...
  void addPlaceholderMessage({
    ...
    _debugLog('Adding placeholder message with ID: $id');
  ...
  void removePlaceholderMessage(String placeholderId) {
    _debugLog('Removing placeholder message with ID: $placeholderId');
  ...
  void searchChats(String query) {
    _debugLog('Searching chats with query: $query');
  ...
  Future<void> updateMessageContent({
    ...
    try {
      _debugLog('Updating message content for messageId: $messageId');

      // Ensure we have a current chat ID
      final chatId = state.currentChatId;
      if (chatId == null) {
        _debugLog('No current chat ID, cannot update message');
        return;
      }
      ...
      if (messageIndex == -1) {
        _debugLog('Message not found: $messageId');
        return;
      }
      ...
      _debugLog('Error updating message content: $e');
  ...
  Future<void> updateMessageMetrics({
    ...
    try {
      _debugLog('Updating message metrics for messageId: $messageId');

      // Ensure we have a current chat ID
      final chatId = state.currentChatId;
      if (chatId == null) {
        _debugLog('No current chat ID, cannot update message metrics');
        return;
      }
      ...
      try {
        ...
        _debugLog('Successfully updated metrics in Firestore');
      } catch (e) {
        // Log but don't propagate error - UI already has the metrics
        _debugLog('Error updating metrics in Firestore (non-critical): $e');
        // Don't emit error state since the local update succeeded
      }
    } catch (e) {
      // Only log errors for metrics updates - don't display to user
      _debugLog('Error updating message metrics in local state: $e');
