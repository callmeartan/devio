...
class GeminiService {
  ...
  Future<LlmResponse> generateResponse({
    ...
    for (final currentModel in modelsToTry) {
      ...
      for (int attempt = 0; attempt < _maxRetries; attempt++) {
        try {
          dev.log(
              'Attempting with model $currentModel (attempt ${attempt + 1})');
          final startTime = DateTime.now();

          final response = await _client
              .post(
                Uri.parse(
                    '$_baseUrl/models/$currentModel:generateContent?key=$_apiKey'),
                headers: {
                  'Content-Type': 'application/json',
                },
                body: jsonEncode({
          ...
          final totalDuration =
              endTime.difference(startTime).inMicroseconds / 1000000;
              ...
              try {
                ...
                if (modelsResponse.statusCode == 200) {
                  ...
                  lastError = LlmResponse(
                    ...
                    isError: true,
                    errorMessage:
                        'Model "$currentModel" not found. Available models: ${availableModels.join(", ")}',
                  );
                } else {
                  lastError = LlmResponse(
                    text: '',
                    isError: true,
                    errorMessage:
                        'Model "$currentModel" not found. Failed to retrieve available models.',
                  );
                }
              } catch (e) {
                lastError = LlmResponse(
                  ...
                  isError: true,
                  errorMessage:
                      'Model "$currentModel" not found. Error retrieving available models: $e',
                );
              }
            } else {
              lastError = LlmResponse(
  ...
  Future<LlmResponse> generateResponseWithImage({
    ...
    List<String> modelsToTry = [
      modelName,
      ..._fallbackVisionModels.where((m) => m != modelName)
    ];
    LlmResponse? lastError;
    ...
    for (final currentModel in modelsToTry) {
      for (int attempt = 0; attempt < _maxRetries; attempt++) {
        try {
          dev.log(
              'Attempting vision model $currentModel (attempt ${attempt + 1})');
          final startTime = DateTime.now();
          ...
          if (response.statusCode == 200) {
            ...
            dev.log(
                'Vision model $currentModel overloaded (attempt ${attempt + 1})');
            lastError = LlmResponse(
              text: '',
              isError: true,
              errorMessage: 'Model overloaded: ${response.body}',
    ...
    try {
      if (!_documentService.isSupportedDocument(file.path)) {
        return LlmResponse(
          ...
          isError: true,
          errorMessage:
              'Unsupported document type. Please provide a PDF or DOCX file.',
        );
      }
  ...
  Future<List<String>> getAvailableModels() async {
    ...
    final cachedAvailableModels = allModels
        .where((model) =>
            _workingModelsCache.containsKey(model) &&
            DateTime.now().difference(_workingModelsCache[model]!) <
                _cacheDuration)
        .toList();
  ...
  List<String> _filterReliableModels(List<String> allModels) {
    ...
    return allModels
        .where((model) =>
            reliableModelPrefixes.any((prefix) => model.startsWith(prefix)))
        .toList();
  }
